...
def _variant_images(pil_rgb: Image.Image, max_w: int = 1920) -> List[Tuple[str, Image.Image]]:
    """Generate preprocessing variants for OCR.

    NOTE: Some call sites pass `max_w=...` (e.g. fast mode). Keep this parameter
    optional so older call sites that don't pass it continue to work.
    """
    # Common resize cap: wide screenshots can be very large; cap width for speed/stability.
    im = pil_rgb
    try:
        mw = int(max_w) if max_w else 0
    except Exception:
        mw = 0
    if mw > 0 and im.width > mw:
        h = int(im.height * (mw / im.width))
